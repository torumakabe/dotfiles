...
try {
    ...
    if ($native.status -ne 'native_fixture_passed_not_install' -or $native.nativeExecuted -ne $true -or
        $native.sacl -ne 'unobserved' -or $native.count -ne 82 -or
        $cases.Count -ne 77 -or @($native.cases | Where-Object status -ne 'passed').Count -ne 0) {
        throw 'Native results do not meet the required 77 fixture and five static checks.'
    }
    $status = 'PASS'
} catch {
...
@{
    ...
    sacl='unobserved'; native=$native
} | ConvertTo-Json -Depth 20 | Set-Content -LiteralPath (Join-Path $output 'result.json') -Encoding utf8
if ($status -eq 'PASS') { Write-Host 'PASS: 77 native fixture cases completed. SACL unobserved. Real migration has not run.' }
