...
- **Copilot sandbox の uv 専用キャッシュ**: macOS/Linux/WSL の Bash tool に対する `uv-enforcer.py` のコマンド内 `UV_CACHE_DIR` 設定と、POSIX 設定同期による専用キャッシュの RW は、CLI/MXC の制約に対するワークアラウンドである。通常シェルの uv 設定と Windows の実行動作は変更しない。CLI または MXC の更新時に、hook による切替なしでも通常の Python 自動探索とホストへのキャッシュ永続化が成功するかを確認し、不要になった処理を整理する。終了コードや policy 表示だけで撤去を判断しない。上流の改善を先取りした移行や廃棄の仕組みは追加しない。ソースを削除しても配布済み hook や RW 許可は残り得るため、適用先も確認し、利用者指定とキャッシュ内容は保持する。実測は `docs/copilot-sandbox-verification.md`、類似の公開報告は [github/copilot-cli#1300](https://github.com/github/copilot-cli/issues/1300) を参照する。同 issue が今回と同一原因であることや修正予定は未確認
