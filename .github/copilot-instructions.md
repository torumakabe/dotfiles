...
- **npm:typescript-language-server の trust_policy_excludes**: `home/dot_config/mise/config.toml.tmpl` で `typescript-language-server@5.3.0` を aube の `trustPolicy=no-downgrade` から除外している。社内 npm プロキシ (Azure DevOps Artifacts) が 5.3.0 の `_npmUser.trustedPublisher` を落とし、証跡を持つ 5.2.0 からの後退と判定されるためである。上流の release workflow は v5.2.0 と v5.3.0 で同一なので欠落はプロキシ側で起きている。プロキシが証跡を保持するようになったら除外を撤去する。別の版で同じ失敗が出たら、パッケージ名だけの除外へ広げず、その版を明記して追加する
