...
各 Hook は host 上の `MISE_ENABLE_TOOLS=uv uv run ...` で実行する。PowerShell も同等であり、runtime の親 PATH から uv の実体を直接起動する。Hook は通常 agent shell の login-shell 環境補完を共有しないため、CLI または GUI の起動元が実体 PATH を継承している必要がある。環境構築は [構造の説明](architecture.md#copilot-の通常-shell-と-command-hook)、切替と復元は [運用手順](operations.md#実体環境への切替) を参照する。
