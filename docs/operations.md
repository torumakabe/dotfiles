...
2. 既存 lockfile を退避
3. `mise upgrade`
4. `minimum_release_age` によるリリース保留以外の `mise WARN` が出力された場合は、既存 lockfile を復元して停止
5. 既存 lockfile を削除し、`mise lock --global --platform ...` で再生成
6. `mise lock` が失敗した場合、またはリリース保留以外の `mise WARN` が出力された場合は、既存 lockfile を復元して停止
7. `chezmoi re-add`
8. git commit + push
...
- `minimum_release_age` によるリリース保留は警告内容を表示して処理を継続する
- 自動処理では、リリース保留以外の `mise WARN` が出力された場合、終了コードが `0` でも commit と push を行わない
- `mise upgrade` または `mise lock` の失敗時は、更新処理を始める前の lockfile を復元する
- 処理を停止した場合は、原因となった警告、lockfile の復元結果、実行ログの保存先を標準エラー出力へ表示する
