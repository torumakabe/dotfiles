...
2. `mise upgrade`
3. `mise WARN` が出力された場合は停止
4. 既存 lockfile を削除し、`mise lock --global --platform ...` で再生成
5. `mise lock` が失敗または `mise WARN` を出力した場合は、既存 lockfile を復元して停止
6. `chezmoi re-add`
7. git commit + push
...
- 自動処理では `mise upgrade` または `mise lock` が `mise WARN` を出力した場合、終了コードが `0` でも commit と push を行わない
- `mise lock` の失敗時は、再生成前の lockfile を復元する
