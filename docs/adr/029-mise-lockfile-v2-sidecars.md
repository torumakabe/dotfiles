...
- PowerShell と zsh の `mise-upgrade` は、target の lockfile と sidecar を一括退避し、両方を削除してから `mise lock --global --platform linux-x64,linux-arm64,macos-arm64,windows-x64,windows-arm64` で再生成する。
