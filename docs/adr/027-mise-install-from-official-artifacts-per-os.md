...
- formula を手動削除する前に、Homebrew 版の activation を読み込んだ既存 shell をすべて終了するか、検証済みの公式 mise 実体から `mise activate zsh` を読み直す。新しい login shell も公式 `mise activate zsh` を使う。共有 global config は `activate_shims = false` で shim farm を `PATH` へ追加せず、`activate_aggressive = true` で mise install path を OS や他のパッケージ管理ツールのディレクトリより前へ置く。共有 profile の公式 `mise env` は、非対話 shell と login 時の環境取得に使う。既存 shell を一つ更新しても、Homebrew の絶対パスを hook に保持する他の shell は安全にならない。cleanup の失敗を警告扱いにする従来の判断は、自動 cleanup 自体を廃止するこの判断で置き換える。
