...
| 2026-09-04 | Windows native、commit `58fcdd0`を隔離した`TEMP`、`TMP`、`UV_CACHE_DIR`で検証 | OS build、architecture未記録 | 1.0.83-5 | 未記録 | mise管理のjqは未確認 | N/A | sandbox設定とplatform parityの43テストが成功し、Windowsで実行しないPOSIXテスト8件をskip。PowerShell関連のfailureとskipは0件。隔離した設定同期ではmise設定のWindows絶対パスを1件だけ追加し、既存filesystem pathの順序、`sandbox.enabled=false`、管理対象外設定を維持した。2回の実行結果はバイト単位で同一。既定の仮想化uv cacheでは一時ファイルの書込拒否が発生したため、テスト用cacheをリポジトリ配下へ隔離した |
| 2026-09-04 | WSL2、commit `38f4804`を適用して新規CLIプロセスで検証 | OS、architecture未記録 | 1.0.83-5、Bubblewrap | 未記録 | mise version未記録、jq 1.8.2 | version未記録 | sandbox設定とplatform parityの43テストが成功し、8件をskip。設定同期はmise設定の絶対パスを`readonlyPaths`へ1件だけ追加し、既存要素と順序を維持した。2回目のapplyで設定は変化しなかった。新規CLIの`uv run`配下で`command -v jq`と`mise which jq`はmise管理下の実体を返し、`mise current jq`は`1.8.2`、`jq --version`は`jq-1.8.2`を返した。終了コードは0で、bypass、rerun、追加readonly grantはなかった。一時ファイルを削除し、作業ツリーがcleanであることを確認した |
